...
*/
		...
		iterator	insert(const value_type& val)
		{
			size_++;
			if (!root)
				return (iterator(root = node_insert(val, root)));
			return (iterator(node_insert(val, root)));
		}
		...
		iterator	insert(iterator position, const value_type& val)
		{
			size_++;
			if (!root)
				return (iterator(root = node_insert(val, position.getNode())));
